...
// Sync member options on all person-type columns AND the private-board Owner dropdown.
...
async function syncOwnerColumn(client, boardId) {
  const colRes = await client.query(
    `SELECT id, settings FROM columns
     WHERE board_id=$1 AND (type='person' OR (title='Owner' AND type='dropdown'))`,
    [boardId]
  );
...
router.post('/', ...canWrite, async (req, res) => {
  const { name, description, visibility = 'org_wide' } = req.body;
  ...
  try {
    ...
    const ownerIsDropdown = visibility === 'private';
    const columnsSpec = [
      {
      ...
      {
        title: 'Owner',
        type: ownerIsDropdown ? 'dropdown' : 'person',
        position: 1,
        settings: ownerIsDropdown ? { options: [creatorName] } : {},
      },
    ...
    // 6. Create 3 default items with empty column values
    const createdItems = [];
    for (let i = 0; i < 3; i++) {
...
router.put('/:id', ...canWrite, async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      ...
      [name, description ?? '', visibility ?? 'org_wide', id]
    );
    res.json(rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
// ── POST add member — also syncs Owner dropdown ───────────────────────────────
router.post('/:id/members', ...canWrite, async (req, res) => {
  ...
  try {
    ...
    // Sync member options on all person/Owner-dropdown columns
...
// ── DELETE remove member — also syncs Owner dropdown ─────────────────────────
